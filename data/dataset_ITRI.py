...
class ImageDataset(dset.ImageFolder):
    ...
    def __init__(self, root_dir, style_A, style_B, load_size, fine_size, phase, opt_choice):
        self.phase = phase
        self.root_dir = root_dir
        self.load_size = load_size
        self.dir_A = os.path.join(root_dir, style_A) + '/{}.jpg'
        self.dir_B = os.path.join(root_dir, style_B) + '/{}.jpg'
        A_paths = pd.read_csv(root_dir + 'itri_day/ImageSets/Main/train.txt', header=None)
        B_paths = pd.read_csv(root_dir + 'itri_night/ImageSets/Main/train.txt', header=None)
        self.A_paths = A_paths.values.tolist()
        self.B_paths = B_paths.values.tolist()
        self.A_paths = random.sample(self.A_paths, 8000)
        self.B_paths = random.sample(self.B_paths, 8000)

        self.A_size = len(self.A_paths)
        self.B_size = len(self.B_paths)
        self.transform = get_transform(
            load_size=load_size, fine_size=fine_size, opt_choice=opt_choice)

    def __getitem__(self, index):
        index_A = random.randint(0, self.A_size - 1)
        A_name = self.A_paths[index_A][0]

        A_path = self.dir_A.format(A_name)

        index_B = random.randint(0, self.B_size - 1)
        B_name = self.B_paths[index_B][0]
        B_path = self.dir_B.format(B_name)
        ...
        return {'A': A_img, 'B': B_img, 'A_name': A_name, 'B_name': B_name}
